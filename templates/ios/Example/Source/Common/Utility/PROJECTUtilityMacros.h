...
#ifndef PROJECTUtilityMacros_h
#define PROJECTUtilityMacros_h
...
#define kIPHONE_X_LATER ([PROJECTDeviceUtility isIphoneXLater])
//状态栏的高度
...
#define AutoFitWidth(val) ([PROJECTDeviceUtility fittedWidth:val])
#define AutoFitHeight(val) ([PROJECTDeviceUtility fittedHeight:val])
...
/* 常用的Block定义 */
typedef void(^CPDVoidBlock)(void);
typedef void(^CPDTapBtnBlock)(UIButton *);
typedef void(^CPDIntegerBlock)(NSInteger value);
typedef void(^CPDBoolBlock)(BOOL value);
typedef void(^CPDArrayBlock)(NSArray *array);
typedef void(^CPDStringBlock)(NSString *string);
typedef void(^CPDStringValueBlock)(BOOL value,NSString *string);
