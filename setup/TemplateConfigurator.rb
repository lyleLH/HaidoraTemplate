...
module Pod
  class TemplateConfigurator
    ...
    def run_pod_install
      needRunPodInstall = ask_with_answers("Would you like to pod install --no-repo-update", ["Yes", "No"]).to_sym
      case needRunPodInstall
        when :yes
    ...
    def reinitialize_git_repo
      `rm -rf .git`
      `curl -L -s https://www.gitignore.io/api/appcode,xcode,objective-c,swift > .gitignore`
      `git init`
      `git add -A`
    end
